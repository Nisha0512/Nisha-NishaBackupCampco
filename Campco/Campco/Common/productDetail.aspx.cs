        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (ProdCode != "")
                {
                    ...
                    // {
                    var path = Server.MapPath(@"~/Pic/" + Prod_Detail.LARGEPIC);
                    var pth = File.Exists(path) ? "../Pic/" + Prod_Detail.LARGEPIC : "../common/assets/images/placeholder.png";
                    Prod_Detail.LARGEPIC = pth;
                    imgsrc = Prod_Detail.LARGEPIC;
                    var path1 = Server.MapPath(@"~/Pic/" + Prod_Detail.EXTRAPIC);
                    var pth1 = File.Exists(path1) ? "../Pic/" + Prod_Detail.EXTRAPIC : "../common/assets/images/placeholder.png";
                    Prod_Detail.EXTRAPIC = pth1;
                    if (Prod_Detail.extraPic.Count > 0)
                    {
                        foreach (var item in Prod_Detail.extraPic)
                        {
                            ...
                            var path15 = Server.MapPath(@"~/Pic/" + item.XlargePic);
                            var pth15 = File.Exists(path13) ? "../Pic/" + item.XlargePic : "../common/assets/images/placeholder.png";
                            item.XlargePic = pth15;

                            if (imgsrc == "")
                            {
                                imgsrc = item.LargePic;
                            }
                            else
                            {
                                imgsrc += "," + item.LargePic;
